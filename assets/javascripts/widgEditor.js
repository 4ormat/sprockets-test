...
var widgStylesheet = "/static/app/widgEditor/widgContent.css";

/* Items to appear in toolbar. */
var widgToolbarItems = new Array();

var widgUserCanEditHtml = (typeof(USER_CAN_EDIT_ASSET_HTML) != "undefined" && USER_CAN_EDIT_ASSET_HTML);
...
if (typeof(USER_CAN_EDIT_ASSET_HTML) != "undefined") {
	...
	if (typeof(document.designMode) == "string" && (document.all || document.designMode == "off"))
	{
		...
		for (var i = 0; i < theTextareas.length; i++)
		{
			...
			if (theTextarea.className.classExists("widgEditor"))
			{
				...
				setTimeout("X = new widgEditor('" + theTextarea.id + "')", 500 * (i));
			}
		}
	}
	else
...
function widg_init(id){
	if (typeof(document.designMode) == "string" && (document.all || document.designMode == "off")){
	  return new widgEditor(id);
	}
}

function widgEditor(replacedTextarea)
{
	var self = this;
	
	this.theTextarea = replacedTextarea;
	if(this.theTextarea == null){
	  return false;
	}
  ...
  this.theExtraInput.value = "true";
	...
	{
		this.theIframe.contentWindow.document.addEventListener("keyup", function(){this.widgEditorObject.dirty = true; return true;}, false);
		this.theTextarea.addEventListener("keyup", function(){this.widgEditorObject.dirty = true; return true;}, false);		
	}
	/* IE event capturing */
	else
	{
		this.theIframe.contentWindow.document.attachEvent("onkeyup", function(){this.widgEditorObject.dirty = true; return true;});
		this.theTextarea.attachEvent("onkeyup", function(){this.widgEditorObject.dirty = true; return true;});
	}

	/* if WYSIWYG is disabled then switch to the textarea right away */
	if (document.getElementById('asset_wysiwyg_disallowed') && document.getElementById('asset_wysiwyg_disallowed').value == 1) {
		this.theContainer.style.visibility = "visible";
		this.theTextarea.style.visibility = "visible";
		
		/* Hide the buttons in the toolbar */
		var theToolbar = this.theToolbar;
		this.hideWysiwygToolbarButtons(theToolbar);
	...
	if(location.pathname.match(/^\/pages\/.+\/edit$/)){Assets.resize();}
	return this;
}
...
widgEditor.prototype.cleanPaste = function()
{
	if (widgAutoClean || confirm("Do you wish to clean the HTML source of the content you just pasted?"))
	{
		...
		var anotherTempNode = $("<div>" + newSnippet + "</div>");
  	// firefox sometimes returns undefined for .innerText, jquery does it right
    newSnippet = anotherTempNode[0].innerText || anotherTempNode.text();
    newSnippet = newSnippet.replace(/(\n|\r|\t)/g, "");
    
		this.theInput.value = matchedHead + newSnippet + matchedTail;
...
widgEditor.prototype.cleanSource = function()
{
  ...
  /* Keep br tags that are just for spacing */
  theHTML = theHTML.replace(/<p><br \/><\/p>/g, "<br />");
	theHTML = theHTML.replace(/<h1><br \/><\/h1>/g, "<br />");	
	theHTML = theHTML.replace(/<h2><br \/><\/h2>/g, "<br />");
	
	/* Replace <b> and <i> tags */
	theHTML = theHTML.replace(/<b><i>|<i><b>/g, "<span style='font-style:italic;font-weight:bold;'>");
	theHTML = theHTML.replace(/<\/i><\/b>|<\/b><\/i>/g, "</span>");
	theHTML = theHTML.replace(/<b>/g, "<span style='font-weight:bold;'>");
	theHTML = theHTML.replace(/<\/b>/g, "</span>");
	theHTML = theHTML.replace(/<i>/g, "<span style='font-style:italic;'>");
	theHTML = theHTML.replace(/<\/i>/g, "</span>");
	...
	/* Remove font tags */
	theHTML = theHTML.replace(/<[\/]?font[^>]*>/g, "");
...
widgEditor.prototype.setParagraph = function(e){
	// check for the editor being inside a list and don't apply <p> if so. Fixes
	// issue where every item in a list was the first item due to each <li> being
	// wrapped in a <p>.
	var orderedListButton = document.getElementById(this.theToolbar.theList.id + "ButtonOrdered");
	var unorderedListButton = document.getElementById(this.theToolbar.theList.id + "ButtonUnordered");
	var listSelected = (orderedListButton.className.match(/ on/) || 
											unorderedListButton.className.match(/ on/));

  if(e.keyCode == 13 && this.wysiwyg && !listSelected){
...
widgEditor.prototype.initEdit = function()
{
	...
	if (typeof document.addEventListener == "function")
	{
		...
		this.theIframe.contentWindow.document.addEventListener("keyup", function(e){self.setParagraph(e); return true;}, false);
	...
	else
	{
		...
		this.theIframe.contentWindow.document.attachEvent("onkeyup", function(e){self.setParagraph(e); return true;}, false);
...
{
	var self = this;
	var theForm = $(this.theContainer).parents('form')[0];
...
{
	if (!this.wysiwyg) {
		var switchModeConfirm = confirm('Switching back to the WYSIWYG editor will erase all content. Are you sure?');
		if (!switchModeConfirm) {
			return false;
		}
	} else {
		var switchModeConfirm = confirm('Once you begin using the HTML editor you cannot switch back to the WYSIWYG editor without erasing all content. Are you sure?');
		if (!switchModeConfirm) {
			return false;
		}
	}
  ...
  this.dirty = true;
	...
	if (!this.locked)
	{
		...
		if (this.wysiwyg)
		{
			...
			document.getElementById('asset_wysiwyg_disallowed').value = 1;
		...
		else
		{
			...
			var theToolbar = this.theToolbar;
			this.showWysiwygToolbarButtons(theToolbar);

			/* If the user is switching back, clear things out */
			this.theTextarea.value = '';
			document.getElementById('asset_wysiwyg_disallowed').value = 0;
...
widgEditor.prototype.showWysiwygToolbarButtons = function(theToolbar)
{
	var theWidgEditor = theToolbar.widgEditorObject;
	for (var i = 0; i < theToolbar.theList.childNodes.length; i++) 
	{
		var theChild = theToolbar.theList.childNodes[i];
		if (theChild.nodeName.toLowerCase() == "li" && theChild.className.match(/widgWysiwyg/))	
		{
			var theAction = theChild.childNodes[0].title.split(' ')[0];
			theWidgEditor.theToolbar.setDisplay(theAction, "show");
		}
	}
	theWidgEditor.theToolbar.setDisplay("SelectBlock", "show");
}

widgEditor.prototype.hideWysiwygToolbarButtons = function(theToolbar)
{
	var theWidgEditor = theToolbar.widgEditorObject;
	for (var i = 0; i < theToolbar.theList.childNodes.length; i++) 
	{
		...
		if (theChild.nodeName.toLowerCase() == "li" && theChild.className.match(/widgWysiwyg/)) 
		{
			...
			if (theAction) {
				...
				theWidgEditor.theToolbar.setDisplay(theAction, "hide");
			}
		}
	}
	theWidgEditor.theToolbar.setDisplay("SelectBlock", "hide");
}
...
widgEditor.prototype.writeDocument = function(documentContent)
{
	...
	// Tyler's hack so keyup events can flag the editor as dirty
	this.theIframe.contentWindow.document.widgEditorObject = this;
	this.theTextarea.widgEditorObject = this;
...
function widgToolbar(theEditor)
{
	...
	for (var i = 0; i < widgToolbarItems.length; i++)
	{
		switch (widgToolbarItems[i])
		{
			...
			case "hyperlink":
				this.addButton(this.theList.id + "ButtonHyperlink", "widgButtonHyperlink", "Hyperlink", "link");
			...
			case "htmlsource":
			  if (document.getElementById('asset_wysiwyg_disallowed')) {
			    var button_class = "widgButtonHTML";
			    if (!widgUserCanEditHtml) {
			      button_class += ' disabled';
			    }
					this.addButton(this.theList.id + "ButtonHTML", button_class, "HTML Source", "html");
				}
				break;
...
widgToolbar.prototype.addButton = function(theID, theClass, theLabel, theAction)
{
	...
	// We hide widgWysiwyg elements when the HTML mode is enabled
	if (theAction != "html") {
	  menuItem.className = menuItem.className + " widgWysiwyg";
	}
...
widgToolbar.prototype.addSelect = function(theID, theClass, theContentArray, theAction)
{
	...
	// We hide widgWysiwyg elements when the HTML mode is enabled	
	menuItem.className = "widgEditSelect widgWysiwyg";
...
/* Hide/show a given item in the toolbar */
widgToolbar.prototype.setDisplay = function(theState, theStatus)
{
	if (theState != "SelectBlock")
	{
		var theButton = document.getElementById(this.theList.id + "Button" + theState);
	...
	switch (this.action)
	{
		...
		case "html":
			if (widgUserCanEditHtml) {
			  ...
			  // Let the event bubble up and get caught by the container's JS
				return true;
			} 
		...
		case "link":
			if (this.parentNode.className.classExists("on"))
			{
				...
				theWidgEditor.theToolbar.setState("Hyperlink", "off");
			}
			else
			{
				...
				{			
					...
					theWidgEditor.theToolbar.setState("Hyperlink", "on");
				}
			}
